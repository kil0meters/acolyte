...
export function renderLinksInElement(element: HTMLElement) {
  element.innerHTML = element.innerHTML.replace(linkRegex, (url: string) => {
    getLinkPreview(url).then((linkElement: HTMLElement) => {
      if (linkElement != null) {
...
export class MessageList {
  ...
  constructor(messageListID: string, maxHeight: number, username: string) {
    this.messageListElement = <HTMLDivElement>(
      document.getElementById(messageListID)
    );
  ...
  buildMessage(message: Message): HTMLElement {
    ...
    if (
      message.text.includes(this.username) &&
    ...
    if (
      this.permissions <= permissions.MODERATOR &&
  ...
  scrollToBottom(offsetHeight: number) {
    if (
      this.messageListElement.clientHeight +
        ...
        64 >=
      this.messageListElement.scrollHeight - offsetHeight
    ) {
  ...
  push(message: Message) {
    ...
    while (this.messageListElement.offsetHeight + 24 > this.maxHeight) {
      // offset for padding
  ...
  checkForCombos() {
    ...
    for (
      let i = 1;
      recentMessage.text === prevMessage.text && i <= this.list.length;
      i++
    ) {
    ...
    if (
      this.currentCombo[this.currentCombo.length - 1].text !==
      recentMessage.text
    ) {
  ...
  replaceComboListWithElement() {
    if (this.currentCombo.length > 1) {
      ...
      let mostRecentMessage = <HTMLDivElement>(
        this.messageListElement.lastElementChild
      );

      if (mostRecentMessage !== null) {
        if (
          Array.from(mostRecentMessage.classList).includes("combo") &&
          mostRecentMessage.firstElementChild.getAttribute("alt") ===
            this.currentCombo[0].text
        ) {
      ...
      currentComboElement.innerHTML = `${replaceTextWithEmotes(
        this.currentCombo[0].text
      )} ${this.currentCombo.length}x ~combo~`;
