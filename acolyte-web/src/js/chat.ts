  ...
  constructor(
    ...
    if (!noEntry) {
      ...
      getEmotes().forEach((emote) => {
        emotePopup.innerHTML += replaceTextWithEmotes(
          emote.name,
          `document.getElementById('entry-body').value += '${emote.name} '`
        );
      });
